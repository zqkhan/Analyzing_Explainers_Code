...
import cxplain
from cxplain import MLPModelBuilder, ZeroMasking, CXPlain, UNetModelBuilder
...
prop_points = 1
calculate = True
# epsilon_range = np.arange(0.01, .120, 0.005)
    ...
    for i in range(run_times):
        ...
        for (j, lambda_dense) in enumerate(lambda_dense_list):
                ...
                model_builder = MLPModelBuilder(num_layers=4, num_units=64, activation=activation, verbose=0,
                                batch_size=16, learning_rate=0.001, num_epochs=20, early_stopping_patience=15,
                                with_bn=False)
                ...
                # explainer = CXPlain(pred_model, model_builder, masking_operation, loss,
                #                     num_models=1, downsample_factors=downsample_factors,
                #                     flatten_for_explained_model=True)
                # explainer.fit(x_train.reshape((len(x_train), 6, 6, 3)), y_train)
